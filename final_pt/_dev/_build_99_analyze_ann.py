...
PROJECT = HERE.parent
OUT = PROJECT / '99_analyze_ann.ipynb'
...
# ── Cell 0: 헤더 ────────────────────────────────────────────────
CELLS.append(md("""# 99_analyze_ann — Black-Litterman LSTM vs ANN 분석

본 노트북은 `final_pt/results/*.pkl` 백테스트 결과를 분석.

## 구조

```
[L14] SET 1~4 통합 paired (전체 기간, 14 pair)
[L15] SET 1~4 × 3 레짐 paired (42 pair)
[L16] SET 5 — P×Q matrix (P=eq/rp × 6 q, 12 pair)
[L17] Portfolio - mcap prior L1 거리
[L18] Q 값 시계열 / 분포 by 레짐
[L20] p_w=eq matrix (prior × q, 18 pair)
[L21] p_w=rp matrix (prior × q, 18 pair)
...
**평가 컷오프**: 2023-12-31 (2024-25 hold-out 제외)
"""))
...
OUT_DIR     = BASE_DIR / 'outputs' / '99_analyze_ann'
...
각 SET pair 를 R1_회복 / R2_확장 / R3_변동 로 분할 평가. 총 14 pair × 3 레짐 = 42 entry.
...
from master_table import REGIMES as _REGIMES
REGIMES = [(lbl, s, e) for lbl, s, e in _REGIMES]
...
for pw, q, label, L, A in PAIRS_L16:
    ...
    rows.append({
        ...
        'L_Sh': mL['Sharpe'], 'A_Sh': mA['Sharpe'], 'd_Sh': mL['Sharpe']-mA['Sharpe'],
        'R1_d_Sh': R_sh['R1_회복'], 'R2_d_Sh': R_sh['R2_확장'], 'R3_d_Sh': R_sh['R3_변동'],
        'd_So': mL['Sortino'] - mA['Sortino'],
        'R1_d_So': R_so['R1_회복'], 'R2_d_So': R_so['R2_확장'], 'R3_d_So': R_so['R3_변동'],
        'd_MDD': mL['MDD'] - mA['MDD'],
...
print(f'{"":<14s}{"R1_회복":>22s}{"R2_확장":>22s}{"R3_변동":>22s}')
print('-'*82)
for metric_lbl, sh_cols, so_cols in [
    ('Sharpe', ['R1_d_Sh','R2_d_Sh','R3_d_Sh'], None),
    ('Sortino', ['R1_d_So','R2_d_So','R3_d_So'], None),
]:
    print(f'  [{metric_lbl}]')
    for pw in ['eq', 'rp']:
...
display_df = df_full.set_index('pair')[[
    'L_Sh','A_Sh','d_Sh','R1_d_Sh','R2_d_Sh','R3_d_Sh',
    'd_So','R1_d_So','R2_d_So','R3_d_So',
    'd_MDD','d_Alp','win']]
display_df.columns = ['LSTM Sh','ANN Sh','Δ Sh (전체)',
                     'Δ Sh (R1)','Δ Sh (R2)','Δ Sh (R3)',
                     'Δ So (전체)','Δ So (R1)','Δ So (R2)','Δ So (R3)',
                     'Δ MDD (전체)','Δ α (전체)','win']

hl_cols = ['Δ Sh (전체)','Δ Sh (R1)','Δ Sh (R2)','Δ Sh (R3)',
           'Δ So (전체)','Δ So (R1)','Δ So (R2)','Δ So (R3)',
           'Δ MDD (전체)','Δ α (전체)']
styled = (display_df.style
    .format({
        'LSTM Sh':'{:.3f}','ANN Sh':'{:.3f}',
        'Δ Sh (전체)':'{:+.3f}','Δ Sh (R1)':'{:+.3f}','Δ Sh (R2)':'{:+.3f}','Δ Sh (R3)':'{:+.3f}',
        'Δ So (전체)':'{:+.3f}','Δ So (R1)':'{:+.3f}','Δ So (R2)':'{:+.3f}','Δ So (R3)':'{:+.3f}',
        'Δ MDD (전체)':'{:+.2%}','Δ α (전체)':'{:+.2%}',
    })
    ...
    for pr, q, L, A in PAIRS:
        ...
        if full_L is None or full_A is None:
            row = {'prior': pr, 'q': q, 'L_Sh': np.nan, 'A_Sh': np.nan, 'd_Sh': np.nan,
                   'L_So': np.nan, 'A_So': np.nan, 'd_So': np.nan, 'win': '?'}
            for lbl in ['R1','R2','R3']:
                for met in ['L_Sh','A_Sh','d_Sh','L_So','A_So','d_So']:
                    row[f'{lbl}_{met}'] = np.nan
            rows.append(row)
            continue
...
for P_W in ['mcap', 'eq', 'rp']:
    ...
    for tag, kor in [('R1','R1_회복'),('R2','R2_확장'),('R3','R3_변동')]:
        ...
        print(f'  [p_w={P_W}] {kor}: Sh LSTM {wL_sh}/{len(sub_sh)} (μ={sub_sh[f"{tag}_d_Sh"].mean():+.3f})'
              f'  |  So LSTM {wL_so}/{len(sub_so)} (μ={sub_so[f"{tag}_d_So"].mean():+.3f})')
    print('='*78)

    # ── Sharpe 표 (절대값 + Δ, 레짐별) ──
    sh_cols = ['L_Sh','A_Sh','d_Sh',
               'R1_L_Sh','R1_A_Sh','R1_d_Sh',
               'R2_L_Sh','R2_A_Sh','R2_d_Sh',
               'R3_L_Sh','R3_A_Sh','R3_d_Sh','win']
    sh_names = ['LSTM Sh','ANN Sh','Δ Sh',
                'L Sh R1','A Sh R1','Δ R1',
                'L Sh R2','A Sh R2','Δ R2',
                'L Sh R3','A Sh R3','Δ R3','win']
    sh_df = df.set_index(['prior','q'])[sh_cols].copy()
    sh_df.columns = sh_names
    sh_hl = ['Δ Sh','Δ R1','Δ R2','Δ R3']
    ...
    so_cols = ['L_So','A_So','d_So',
               'R1_L_So','R1_A_So','R1_d_So',
               'R2_L_So','R2_A_So','R2_d_So',
               'R3_L_So','R3_A_So','R3_d_So','win']
    so_names = ['LSTM So','ANN So','Δ So',
                'L So R1','A So R1','Δ R1',
                'L So R2','A So R2','Δ R2',
                'L So R3','A So R3','Δ R3','win']
    so_df = df.set_index(['prior','q'])[so_cols].copy()
    so_df.columns = so_names
    so_hl = ['Δ So','Δ R1','Δ R2','Δ R3']
...
# 전체 TEST (참고)
display(Markdown('### 전체 TEST (2010-01 ~ 2023-12, 168mo)'))
rL = _anchor_ret_metrics(ANCHOR_L, '2010-01-01', '2023-12-31')
rA = _anchor_ret_metrics(ANCHOR_A, '2010-01-01', '2023-12-31')
...
# ── (A) OWN sample 표 ──
rows_own = []
for lbl, s, e in REGIMES + [('전체','2010-01-01','2023-12-31')]:
