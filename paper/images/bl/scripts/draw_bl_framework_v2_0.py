...
# ------------------------------------------------------------------------------
# [1열: 전망 비중 p] (x: 4.3 ~ 6.0, 중심 x = 5.15)
# ------------------------------------------------------------------------------
...
ax_main.text(5.15, 8.05, r"$p_i^{\mathrm{mcap}} = \pm \frac{m_i}{\sum_{\mathcal{G}} m_j}$", ha='center', fontsize=8.2, color=DARK_BLUE, weight='bold')
ax_main.text(5.15, 7.55, r"$p_i^{\mathrm{eq}} = \pm \frac{1}{n_g}$", ha='center', fontsize=8.2, color=DARK_BLUE, weight='bold')

# rp 리스크 패리티 단일 수식 통일 (지수 감마 도입)
ax_main.text(5.15, 6.9, r"$p_i^{\mathrm{rp}} = \pm \frac{\hat{\sigma}_i^{\gamma_i}}{\sum_{\mathcal{G}} \hat{\sigma}_j^{\gamma_i}}$", ha='center', fontsize=8.2, color=DARK_BLUE, weight='bold')
ax_main.text(5.15, 6.45, r"$(\gamma_i = \mp 1)$", ha='center', fontsize=8.0, color='#37474f', weight='bold')
...
# 이미지 출력 및 보존 (Workspace images/bl 폴더에 v2_0으로 저장!)
output_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
output_path = os.path.join(output_dir, "bl_framework_v2_0.png")

plt.savefig(output_path, dpi=300, bbox_inches='tight')
plt.close()

print(f"Publication-ready V2.0 diagram successfully generated at: {output_path}")
