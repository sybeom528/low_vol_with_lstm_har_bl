...
> **📌 사후 정정 (2026-05-12, About 페이지 본문 구현 중)**:
> 사용자 검토 결과 음역 표현 ("어댑티브 볼컨트롤") 이 한국어 사용자에게 어색하다는 피드백 발생.
> Dashboard 의 기존 키워드 (Overview 차별화 카드의 "변동성 인지 자산배분") 와 정합되는 표현으로 변경.
> 영문 표기 (`Adaptive Volatility Control Fund`) 는 그대로 유지 (학술 / 영문 일관성).
> 자세한 이력: `updatelog.md` 의 "B-2 펀드 한글명 변경" 항목 참조.

**결정**: ~~`어댑티브 볼컨트롤 펀드`~~ → **[정정 2026-05-12: `변동성 인지 적응 펀드`]**

**근거**:
- (원본 2026-05-10) 영문 음역 (Adaptive → 어댑티브, VolControl → 볼컨트롤) 으로 일관성 유지. 한국어 의역 ("적응형 변동성 통제 펀드") 은 길어서 가독성 ↓.
- **(정정 2026-05-12)** 음역이 일반 사용자에게 어색. Dashboard 의 기존 키워드 ("변동성 인지 자산배분") 와 정합되는 의역 표현으로 변경. 길이도 10자로 음역 (11자) 보다 짧음.

### 결과 / 함의

- 모든 페이지 헤더에 한/영 병기 표시 (예: ~~"Adaptive VolControl Fund / 어댑티브 볼컨트롤 펀드"~~ → **[정정 2026-05-12: "Adaptive Volatility Control Fund / 변동성 인지 적응 펀드"]**)
