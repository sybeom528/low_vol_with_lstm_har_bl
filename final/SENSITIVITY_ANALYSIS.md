...
본 문서는 [99_analyze.ipynb](99_analyze.ipynb) **섹션 M (Q sensitivity, M1~M3)** 와 **섹션 N (PCT_GROUP sensitivity, N1~N4)** 의 winner 슬롯 민감도 분석 파이프라인이 **왜 필요한가**, **각 단계가 무엇을 답하는가**, **어디까지 해야 충분한가**를 정리.
...
### ✅ N3 실측 결과 (2026-05-08) — Winner-favorable signal

**관찰값**:
```
Jobson-Korkie 5% 유의:    4/6
Jobson-Korkie Bonferroni: 2/6  ← pct=0.10, pct=0.15 (둘 다 winner가 유의 우위)
Bootstrap CI 겹침:        7/7  ← 0.20~0.40 모든 pct가 winner CI와 겹침
```

**패턴 진단**: **Winner-favorable — winner가 좁은 cut 대비 sharpe 유의 우위 + plateau ([0.20, 0.40]) 내 동등**

#### 결과 요약

- **winner를 쓸 수 있는 영역** = [0.20, 0.40] (sharpe·sortino 모두 winner와 동등)
- **winner를 쓰면 안 되는 영역** = 좁은 cut(≤0.15) — winner 대비 **유의 열등**
- → **winner 30%는 plateau 중앙 + 좁은 cut 대비 통계적 우위 입증**

| pct | 검정 결과 | 사용 여부 |
|---|---|---|
| 0.10 | winner 대비 sharpe ★★ 유의 열등 | ❌ |
| 0.15 | winner 대비 sharpe ★★ 유의 열등 | ❌ |
| 0.20, 0.25 | winner와 동등 | ✅ 공격형 후보 |
| **0.30 (winner)** | 기준 | ✅ 중립형 default |
| 0.35, 0.40 | winner와 동등 | ✅ 안정형 후보 |

#### 메커니즘 (왜 좁은 cut이 열등?)

좁은 cut(48~72종목씩 long/short)은 specific risk가 커서 전체 변동성↑ → sharpe 분모 페널티. 하방 위험에선 차이 못 잡아내지만(sortino bootstrap 7/7 겹침), 분산 효과가 부족한 영역이라는 점은 명확.

#### narrative

> "PCT sensitivity 분석은 winner 30% 선택을 통계적으로 지지한다. winner는 좁은 cut(0.10·0.15) 대비 Bonferroni 보정 후도 sharpe 유의 우위(★★)를 보이며, 분산 충분 영역 [0.20, 0.40]에서는 모든 pct가 winner와 동등(bootstrap CI 7/7 겹침). 즉 winner는 **plateau 중앙 + 좁은 cut 대비 입증된 우위** 두 측면에서 정당화된다."

#### 성향별 ETF 매핑

| 성향 | pct | 평가 |
|---|---|---|
| 공격형 | **0.20-0.25** | ✅ winner와 동등, 안전한 차별화 |
| 중립형 | **0.30 (winner)** | ✅ default |
| 안정형 | **0.35-0.40** | ✅ winner와 동등 + 분산 안정 보너스 |

→ **3 성향 매핑 그대로 유효**. 좁은 cut(0.10~0.15)은 어차피 specific risk 우려로 사용 안 하는 영역이며, N3가 이를 통계적으로 추가 확인.
...
본 분석은 **각각 독립** sensitivity (1D). 결합 효과(2D grid)는 미수행. 가능한 인터랙션:
- 작은 q + 큰 pct: "보수적 분산" — 안정형 ETF
- 큰 q + 작은 pct: "집중 베팅" — 공격형 ETF
- 본 winner (q=0.003 + pct=0.30): "균형형" — 중립형

## 3-3. 성향별 ETF 차별화 매핑 (제안)

| 성향 | q | pct | 근거 |
|---|---|---|---|
| **공격형** | 0.005 | 0.20-0.25 | R2 alpha 극대화 + 집중 베팅 |
| **중립형** | **0.003 (winner)** | **0.30 (winner)** | 양 trade-off 균형 |
| **안정형** | 0.001-0.002 | 0.35-0.40 | R3 보호 + 분산 안정 |

→ **q × pct 두 축 차별화로 3 성향 ETF 라인업 자연스럽게 구성**.
...
> "winner pct=0.30은 [0.20, 0.40] 영역의 plateau에서 robust (Bootstrap Sortino CI 7/7 모두 winner와 겹침). 추가로 좁은 cut(0.10·0.15) 대비 Bonferroni 보정 후도 sharpe 유의 우위(★★) → **plateau 중앙 + 좁은 cut 대비 통계적 우위** 두 측면에서 정당화. BAB 학술 표준(30%)과 일치."
...
### H. PCT-risk profile differentiation
> "안정형 ETF: pct=0.35-0.40 (분산 안정, N3 winner와 동등), 공격형: pct=0.20-0.25 (집중 신호, N3 winner와 동등). 좁은 cut(0.10~0.15)은 specific risk로 winner 대비 sharpe 유의 열등 — 어차피 사용 안 하는 영역. winner 30%은 중립형 표준."
...
> - **PCT**: plateau ([0.20, 0.40]) 내 모든 pct가 winner와 동등 + 좁은 cut(0.10·0.15) 대비 sharpe 유의 우위
>
> → 두 축에서 동시에 plateau 중앙 위치 + 비교 영역에서 통계적 우위 입증."

### J. Risk profile from two axes (실측 반영)
> "성향별 ETF 라인업: 안정형(q=0.001, pct=0.35-0.40), 중립형(q=0.003, pct=0.30 = winner), 공격형(q=0.005, pct=0.20-0.25). 모든 성향 후보가 N3 검정에서 winner와 통계적 동등 → 두 축으로 자연스러운 차별화."
...
| Information Ratio vs SPY | 액티브 운용 품질 | 중 (Phase 2 holdout) |
...
**관련 노트북**: [99_analyze.ipynb](99_analyze.ipynb) §M, §N
